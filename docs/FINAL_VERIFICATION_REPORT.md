...
| Focused changed-path tests | Pass | Latest ledger/processing/autonomy pass: `91 passed`. It proves bounded compact document snapshots, selected-review filtering and ordering, duplicate-plan behavioral equivalence, and no fallback to per-pair full-history reads. Earlier readiness, recovery, API rendering, redaction, runtime identity, and support suites also passed. |
| Full backend tests | Pass | Final CI-equivalent source rerun: `797 passed`, `4 skipped`. This includes compact duplicate reassessment, quiescent ledger/source recovery, tamper/collision/rollback handling, maintenance mutation guards, dependency-safe workflow continuation, authoritative worker ownership, governed vendor templates, private OCR preprocessing/cleanup, encrypted runtime secrets, managed cloud ownership/status, document-group review pagination, authoritative queue-wide safety totals, shared readiness lifecycle, bounded caches/projections, and release packaging safety. |
...
| Control-center data path | Pass | Real 150-work-order traffic fell from 988,287 to 141,040 bytes; master-ledger traffic fell from 202,949 to 918 bytes with the same checksum. Duplicate reassessment now uses one compact snapshot instead of 32 full document-history reads: its median fell from about 206 ms to 22.95 ms, autonomy counts from 252.56 ms to 118.67 ms, and direct plan generation from 485.89 ms to 336.69 ms. The restarted authenticated HTTP plan measured 217.94 ms median / 262.25 ms p95. |
...
| Desktop/narrow browser QA | Pass on latest UI-changing baseline | The unchanged dashboard previously rendered in the in-app Browser with the live 119-document queue, exact `50 -> 100 -> 119` incremental loading, and no horizontal/control-text overflow or console errors at desktop/responsive breakpoints. For this backend-only optimization, the rebuilt frontend passed all 165 tests and live HTTP identity, but a repeat Browser DOM capture was unavailable because the control bridge returned a stale tab identifier. No new manual browser result is claimed. |
...
| Remote CI | Pass | GitHub Actions run `31329640244` on compact duplicate-reassessment implementation commit `a6ce7c4` passed the frontend, Linux backend, and all four Windows backend jobs. |
