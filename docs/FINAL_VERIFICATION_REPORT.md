...
| HAI contract | Pass locally | Standard Windows mode exposes 13 governed commands and eight read-only resources. Maintenance mode exposes zero commands while retaining recovery status as a read-only resource; restore is never a HAI command. A read-only command plan succeeded with `externalSubmission=not_executed`; no provider mutation was attempted. |
