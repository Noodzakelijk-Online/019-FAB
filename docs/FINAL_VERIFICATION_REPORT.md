...
| Focused changed-path tests | Pass | Four activation tests prove dependency ordering, reauthorization handling, unavailable review counts, and evidence-only review treatment. Five review-draft tests prove exact-identity restore, stale/future/malformed rejection, size bounds, change detection, and explicit discard. |
| Full backend tests | Pass | Final CI-equivalent source rerun: `801 passed`, `4 skipped`. This includes consistent read snapshots, compact exception/delivery projections, duplicate reassessment, quiescent ledger/source recovery, tamper/collision/rollback handling, maintenance mutation guards, dependency-safe workflow continuation, authoritative worker ownership, governed vendor templates, private OCR preprocessing/cleanup, encrypted runtime secrets, managed cloud ownership/status, document-group review pagination, authoritative queue-wide safety totals, shared readiness lifecycle, bounded caches/projections, and release packaging safety. |
| Web type check | Pass | `pnpm check`. |
| Web tests | Pass | 19 files, `174 passed`; this includes activation and draft invariants, public/private API address separation, browser-origin validation, recovery-policy redaction, bounded review-page projection, local-operator authentication isolation, real TCP tests for nested SPA routing, production security headers, static caching, operations auth/error envelopes, and disabled-by-default billing. |
...
| Desktop/narrow browser QA | Pass on current source | A fresh in-app Browser reload rendered the live 119-document queue and activation flow at `http://127.0.0.1:3005/admin/operations`. Live readiness selected the stale receipt executor as step 4 of 5 and the primary action opened that exact drawer. A source-backed review draft survived close/reopen and a full reload, then explicit discard restored the original form without submitting. Desktop and narrow activation checks found no page-level or activation overflow, and console warning/error reads stayed empty. |
...
| Remote CI | Pass | GitHub Actions run `31333616656` on guided activation and review-draft implementation commit `cc718f3` passed the frontend, Linux backend, and all four Windows backend jobs. |
