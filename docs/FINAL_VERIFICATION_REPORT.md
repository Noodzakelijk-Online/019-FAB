...
| Full backend tests | Pass | Final source rerun: `815 passed` plus 38 subtests. This includes authoritative one-shot worker ownership, private OCR preprocessing/cleanup, preprocessing audit evidence, encrypted runtime secrets, managed cloud ownership/status, lazy dependency loading, bounded backup caching, and compact review response coverage. |
