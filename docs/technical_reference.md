...
*   **Overlapping Runs**: An atomic local run lock prevents cron, Task Scheduler, or repeated HTTP triggers from executing the workflow concurrently on one host. Locks older than `workflow_run_lock_stale_seconds` are treated as abandoned. Multi-host deployments still require a distributed scheduler or database-backed lease.
...
*   **Local Wave Control Center**: `LocalWaveControlService` exposes the modeled Wave menu/action catalog, a structured Reports registry, credential presence without secret values, and policy-gated operation planning through `/api/wave`, `/api/wave/actions`, `/api/wave/reports`, `/api/wave/reports/plan`, `/api/wave/report-snapshots`, `/api/wave/plan`, and `/api/wave/workflows/plan`. It plans general-ledger, trial-balance, sales-tax, customer/vendor, and period-close report workflows with deterministic operation IDs, persists Wave report snapshots for report type, period, basis, account/contact scope, export format, safety, and workflow provenance, but marks every plan as `externalSubmission=not_executed` until a separate approved Wave API or browser executor is attached.
...
*   **Local Source Registry**: `LocalOperationsLedger` keeps `source_accounts` records for observed folders and connector sources. Local folder rescans update source status, last scan time, seen/imported/duplicate counters, and document `source_account_id` links. `/api/sources` lists source provenance, and `POST /api/sources` lets future connectors register safe source metadata. Metadata keys that look like tokens, passwords, secrets, credentials, authorization headers, or API keys are redacted before persistence.
*   **Local Document Processing**: Imported ledger documents can be processed from the dashboard/API. Text files are parsed locally, other files use the configured processor pipeline, and extracted fields, category confidence, validation results, review gates, and audit events are written back to SQLite. Each processing run also refreshes `extracted_fields` rows for vendor, date, amount, VAT/BTW, currency, line items, and category with field-level confidence and provenance so review screens and downstream learning can inspect evidence without scraping raw OCR text.
...
*   **Local Export Attempts**: `LocalExportAttemptService` turns prepared routing attempts into durable `export_attempts` rows with Wave target system, surface/action, operation id, payload snapshot, approval status, external-submission state, result evidence, and redacted metadata. The dashboard Export Attempts panel and `/api/export-attempts` endpoints can prepare approval records, require the exact phrase `APPROVE FAB EXPORT DRAFT` before marking an export locally approved, and require `RECORD FAB EXPORT RESULT` before recording a separate executor result. Approval never submits to Wave by itself; it only updates FAB's source-of-truth ledger and audit trail.
...
*   **Local Operations Health**: `LocalOperationsHealth` converts ledger state into an actionable health summary for `/api/health` and the dashboard. It flags stale review items, stuck imported/processing/review documents, failed documents, routing blocks, prepared routing drafts waiting too long for approval/export, stale running workflow runs, and failed workflow runs. Thresholds are configurable through `[operations] review_stale_hours`, `document_stale_hours`, `routing_stale_hours`, and `workflow_stale_hours`.
...
*   **Local Autonomous Cycle**: `LocalAutonomousService` powers `/api/autonomy/plan`, `/api/autonomy/run`, and the Autonomous Cycle dashboard panel. It combines readiness, operations health, intake state, review queues, routable documents, pending routing drafts, imported bank transactions, reconciliation candidates, and Wave workflow planning into one policy-gated run loop. Executable actions are limited to local folder rescans, imported-document processing, Wave draft preparation without external submission, reconciliation candidate creation from persisted or supplied bank transactions, and read-only Wave daily reconciliation planning. Remote exposure blocks, health blocks, review decisions, draft approvals, Wave submission, credential changes, backup restore, deletion, and outbound messages remain outside the autonomous executor and require explicit review/approval.
