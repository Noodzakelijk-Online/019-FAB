...
*   **Purpose**: Retains the legacy Photos Library fetcher for compatibility only. Google's removed whole-library scopes make unattended library scanning unavailable; the local connector control plane therefore reports `supervision_required` and never invokes it in the worker.
*   **Dependencies**: `google-api-python-client`, `google-auth-oauthlib`, `requests`.
*   **Configuration**: New installations use `google_photos_mode=picker`, `google_photos_credentials_file`, and `google_photos_picker_token_file`. A future supervised Picker-session executor must register only media the user selects; background whole-library access is not presented as available.
...
*   **Local Connector Intake and Source Registry**: `LocalConnectorIntakeService` independently plans and synchronizes explicitly enabled Gmail, Google Drive, and Freshdesk sources before autonomous processing. Each run writes a workflow record, source-account status/counters, provider provenance, document evidence, exact-content duplicate state, provider-side revision review work, and audit evidence while keeping `externalSubmission=not_executed`. A failure in one connector cannot suppress the others, and partial runs preserve already downloaded evidence. Google Photos is fail-closed as `supervision_required` until a Picker-session executor is implemented. `/api/sources/readiness`, `POST /api/sources/sync`, the Sources dashboard panel, and the worker expose this control plane; `/api/sources` retains the observed registry. Secret-like metadata and errors are recursively redacted before persistence.
