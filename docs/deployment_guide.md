...
6.  **Run the Local Operations API (optional):**
    ...
    ```
    Open `http://127.0.0.1:5001/` for the local dashboard. The Export Attempts panel is the posting source of truth: it requires `APPROVE FAB EXPORT DRAFT`, uses an atomic execution claim and pre-execution backup, and requires `RECORD FAB EXPORT RESULT` for supervised outcomes. MijnGeldzaken execution writes a local checksum-bound artifact and remains `supervision_required` until the user-owned session result is recorded. `/api/health` reports stuck execution claims and pending supervision. Keep the host on `127.0.0.1` for local use. If you expose it through ngrok or bind it to anything other than loopback, configure `api_token`; the API refuses non-loopback exposure without a token. The complete endpoint inventory is available from the dashboard and technical reference.
