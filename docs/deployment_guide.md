...
5.  **Run the Application:**
    ...
    ```
    The worker runs legacy connector ingestion, the policy-gated local autonomous cycle, scheduled local report generation, local notification refresh, operations-ledger export processing, and optional compatibility retries as independent audited stages. One failed connector, report, or notification stage does not suppress the remaining local bookkeeping stages. Set `worker_run_legacy_workflow=false` for local-folder-only installations that do not configure the legacy Gmail/Drive/Photos/Freshdesk pipeline. `worker_interval_seconds` controls the interval; `worker_run_once=true` is useful when Windows Task Scheduler supplies the recurrence. A SQLite runtime lease prevents the worker, Task Scheduler, and `/api/autonomy/run` from overlapping the local autonomous cycle. Scheduled reports use a separate unique database slot, so overlapping worker launches cannot create duplicate report artifacts. Notification refresh uses stable issue fingerprints, so repeated cycles update one inbox item instead of creating alert noise. Expired autonomy leases recover after `fab_autonomy_lease_seconds`.
