...
The operator dashboard initially loads 50 complete document reviews. Use **Load next** to retrieve the next bounded page; the status shows loaded versus authoritative total. FAB keeps every open decision for one document together, and the activation checklist always uses queue-wide posting blockers even when only the first page is visible.
