...
class WaveappsPersonalHandler(BaseDataEntryHandler):
    """Handles data entry into a Waveapps personal account via API or CSV fallback."""
    ...
    def _create_expense_via_api(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "failure", "message": f"Waveapps Personal API request failed: {exc}", "requires_manual_review": True}
    ...
    def enter_data(self, categorized_data: Dict[str, Any]) -> Dict[str, Any]:
        if not self.access_token or not self.business_id:
            ...
            csv_filename = f"waveapps_personal_import_{document_id}.csv"
