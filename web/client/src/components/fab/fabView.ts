...
export function statusTone(value: unknown): "good" | "warn" | "bad" | "neutral" | "info" {
  ...
  if (["blocked", "blocked_by_review", "source_file_unavailable", "failed", "error", "disconnected", "high", "unavailable", "invalid"].includes(status)) return "bad";
  if (["attention", "needs_attention", "needs_auth", "needs_authorization", "needs_processing", "needs_review", "refresh_wave_readback", "medium", "due", "deferred", "prepared_disabled", "not_configured", "supervision_required", "incomplete", "legacy_ledger_only"].includes(status)) return "warn";
