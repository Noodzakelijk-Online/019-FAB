      ...
      <aside className="fab-detail-drawer fab-drive-setup-drawer" role="dialog" aria-modal="true" aria-labelledby="fab-gmail-setup-title" aria-describedby="fab-gmail-setup-description">
        ...
        <div className="fab-detail-body">
          ...
          <div className="fab-drive-setup-progress">
            ...
            <SetupStep complete={gmailAuthorized} active={authorizationInProgress} icon={ShieldCheck} title={copy("Read-only Gmail consent", "Alleen-lezen Gmail-toestemming")} detail={authorizationInProgress ? copy("Complete consent in the browser window", "Voltooi toestemming in het browservenster") : reauthorizationRequired ? copy("Fresh consent required; reuse the installed OAuth client", "Nieuwe toestemming vereist; gebruik de geinstalleerde OAuth-client opnieuw") : tokenPresent ? text(authorization.emailAddress, copy("Local token present", "Lokaal token aanwezig")) : copy("Authorization required", "Autorisatie vereist")} />
          </div>
          ...
          <section className="fab-drive-credential-panel">
            ...
            {credentialsPresent && <p>{copy("The installed client can be reused for fresh consent. Choose another JSON only when you intentionally rotate the OAuth client.", "De geinstalleerde client kan opnieuw worden gebruikt voor nieuwe toestemming. Kies alleen een andere JSON wanneer je de OAuth-client bewust roteert.")}</p>}
