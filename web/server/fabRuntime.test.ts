...
describe("FAB runtime identity", () => {
  it("exposes an exact, non-cacheable service marker and API endpoint", async () => {
    const app = express();
    registerFabRuntimeRoute(app, "http://127.0.0.1:5007", "/app");
    ...
    try {
      ...
      let expectedRoot = path.resolve("/app").replaceAll("\\", "/").replace(/\/+$/, "");
      if (process.platform === "win32") expectedRoot = expectedRoot.toLowerCase();
      await expect(response.json()).resolves.toEqual({
        service: "fab-operator-dashboard",
        apiVersion: "1",
        localApiEndpoint: "http://127.0.0.1:5007",
        instanceId: createHash("sha256").update(expectedRoot, "utf8").digest("hex"),
      });
    } finally {
